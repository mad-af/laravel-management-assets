<form wire:submit="save" class="space-y-2">
    ...
    {{-- Asset --}}
    <div>
        <livewire:components.combobox name="assets" wire:model.live="asset_id" :options="$assets" option-value="id"
            option-label="name" option-sub-label="tag_code" option-meta="code" option-avatar="image" label="Kendaraan"
            placeholder="Pilih kendaraan" required class="input-sm" />
    </div>
    ...
    @if ($asset_id)
        ...
        {{-- Due Date --}}
        <div>
            <x-checkbox label="Aktifkan Pajak Tahunan" wire:model.live="is_pajak_tahunan"
                hint="Centang untuk mengaktifkan pajak tahunan" />
        </div>
        ...
        <div class="flex gap-2 justify-end pt-4">
            ...
            <x-button :label="$isEdit ? 'Update' : 'Simpan'" class="btn-sm btn-primary" type="submit" spinner="save" :disabled="!$this->isConfirmed" />
        </div>
    @endif
</form>