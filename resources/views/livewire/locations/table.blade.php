<div class="shadow card bg-base-100">
  <div class="card-body">
    ...
    <div class="flex flex-col gap-4 mb-4 sm:flex-row">
      {{-- Search Input --}}
      <div class="flex-1">
        <x-input wire:model.live="search" placeholder="Cari lokasi..." icon="o-magnifying-glass" class="input-sm" />
      </div>
    ...
    <div>
      ...
      @endphp
      <x-table :headers="$headers" :rows="$locations" striped show-empty-text>
    ...
    @if($locations->count() > 0)
      <div class="flex flex-col gap-4 mt-4 lg:flex-row lg:items-center lg:justify-between">
        <div class="text-sm text-base-content/70">
          Menampilkan {{ $locations->firstItem() }}-{{ $locations->lastItem() }} dari {{ $locations->total() }}
          lokasi
        </div>
