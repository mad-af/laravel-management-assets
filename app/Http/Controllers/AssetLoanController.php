...
class AssetLoanController extends Controller
{
    ...
    {
        $query = AssetLoan::with(['asset', 'employee']);
    ...
    {
        $assets = Asset::where('status', '!=', AssetStatus::MAINTENANCE)
            ->whereNotIn('status', [AssetStatus::MAINTENANCE, AssetStatus::LOST, AssetStatus::MAINTENANCE])
            ->orderBy('name')
            ->get();
    ...
    public function update(Request $request, AssetLoan $assetLoan): RedirectResponse
    {
        ...
        if ($validated['checkin_at'] && ! $assetLoan->getOriginal('checkin_at')) {
            ...
            if (isset($validated['condition_in'])) {
                if (in_array($validated['condition_in'], [AssetCondition::POOR->value])) {
            ...
            $assetLoan->asset->update(['status' => AssetStatus::MAINTENANCE]);
        }
